...
from .fairchem_old_base import GraphModelMixin
...
from .so3 import SO3Linear, SO3Rotation
...
class EquiformerV3FlowMap(torch.nn.Module, GraphModelMixin):
    """
    Args:
    ...
    def __init__(
        ...
        self.sphere_embedding = torch.nn.Embedding(
            self.max_num_elements, self.num_channels
        )

        # Time embedding
        self.time_embedding = NoiseLevelEncoding(self.num_channels)
        self.atom_time_mixer = nn.Linear(3 * self.num_channels, self.num_channels)
        ...
        # Envelope function
        self.envelope_func = (
            PolynomialEnvelope(cutoff=self.cutoff, exponent=5)
        ...
        self.so3_rotation = SO3Rotation(
            self.lmax, self.mmax, use_rotation_mask=(not self.direct_prediction)
        )
        ...
        # Output blocks for forces (and optionally stress)
        self.norm = get_normalization_layer(
            self.norm_type, lmax=self.lmax, num_channels=self.num_channels
        )
    ...
    def _forward_edge(self, edge_distance, edge_distance_vec):
    ...
    def _forward_embedding(
        self, atomic_numbers, s, t, edge_distance, edge_index, edge_envelope_weight
    ):
        assert atomic_numbers.shape == s.shape == t.shape
        ...
        # Time embeddings
        s_embedding = self.time_embedding(s)
        t_embedding = self.time_embedding(t)
        atom_embedding = self.atom_time_mixer(
            torch.cat([atom_embedding, s_embedding, t_embedding], dim=-1)
        )
        x[:, 0, :] = atom_embedding
    ...
    def forward(self, data, s, t):
        ...
        edge_distance, edge_envelope_weight = self._forward_edge(
            edge_distance, edge_distance_vec
        )
        x = self._forward_embedding(
            atomic_numbers,
            s.repeat_interleave(data.natoms),
            t.repeat_interleave(data.natoms),
            edge_distance,
            edge_index,
            edge_envelope_weight,
        )
        ...
        if self.regress_forces:
            ...
            outputs = outputs + (forces,)

        # Stress Prediction
        if self.regress_stress:
            stress = self.stress_block(x, batch_size=self.batch_size, batch=data.batch)
            outputs = outputs + (stress.view(-1, 3, 3),)
    ...
    def _init_edge_rot_mat(self, edge_distance_vec):
        return init_edge_rot_mat(
            edge_distance_vec, use_rotation_mask=(not self.direct_prediction)
        )
    ...
    def _init_weights(self, m):
        if isinstance(m, torch.nn.Linear) or isinstance(m, SO3Linear):
