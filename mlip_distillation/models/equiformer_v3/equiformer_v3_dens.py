...
class EquiformerV3DeNS_OC(EquiformerV3_OC):
    ...
    def __init__(
        ...
        self.force_embedding = SO3Linear(
            in_features=1, out_features=self.num_channels, lmax=self.lmax, bias=True
        )
    ...
    def _forward_direct(self, data):
        ...
        if self.regress_forces:
            ...
            outputs["forces"] = (
                forces * (~noise_mask_tensor) + denoising_pos_vec * noise_mask_tensor
            )
        ...
        if self.regress_stress:
            ...
            outputs["stress"] = stress * (
                ~dens_batch_mask_tensor
            )  # not predict stress during DeNS
    ...
    def _forward_gradient(self, data):
        ...
        if self.regress_forces:
            ...
            outputs["forces"] = (
                forces * (~noise_mask_tensor) + denoising_pos_vec * noise_mask_tensor
            )  # not predict forces during DeNS
    ...
    def _generate_dens_data(self, data):
        ...
        if hasattr(data, "denoising_pos_forward") and data.denoising_pos_forward:
            ...
            if hasattr(data, "noise_mask"):
                ...
                noise_mask_tensor = torch.ones(
                    (num_atoms, 1), dtype=torch.bool, device=self.device
                )
            if hasattr(data, "dens_batch_mask"):
                dens_batch_mask_tensor = data.dens_batch_mask.view(-1, 1)
            else:
                dens_batch_mask_tensor = torch.ones(
                    (len(data.natoms), 1), dtype=torch.bool, device=self.device
                )
            dens_mask_tensor = torch.ones((1, 1), dtype=torch.bool, device=self.device)
        else:
            force_data = torch.zeros(
                (num_atoms, 3), dtype=self.dtype, device=self.device
            )
            noise_mask_tensor = torch.zeros(
                (num_atoms, 1), dtype=torch.bool, device=self.device
            )
            dens_batch_mask_tensor = torch.zeros(
                (len(data.natoms), 1), dtype=torch.bool, device=self.device
            )
            dens_mask_tensor = torch.zeros((1, 1), dtype=torch.bool, device=self.device)
        force_sh = e3nn.o3.spherical_harmonics(
            l=self.irreps_sh, x=force_data, normalize=True, normalization="component"
        )
