...
def get_activation(
    act_name, lmax, mmax, grid_resolution_list=None, use_m_primary=False
):
    """
...
def add_dropout(act, drop):
    ...
    for attr_name in attribute_name_list:
        if attr_name == "gate_act" and isinstance(
            act, SeparableGateS2Activation_SwiGLU_Merge
        ):
        ...
        if hasattr(act, attr_name):
            ...
            update_act_list = [temp, torch.nn.Dropout(drop)]
...
class GateActivation(torch.nn.Module):
    ...
    def extra_repr(self):
        return "lmax={}, mmax={}, use_m_primary={}".format(
            self.lmax, self.mmax, self.use_m_primary
        )
...
class S2Activation(torch.nn.Module):
    def __init__(self, lmax, mmax, grid_resolution_list=None, use_m_primary=False):
        ...
        self.so3_grid = SO3Grid(
            self.lmax,
            self.mmax,
            resolution_list=grid_resolution_list,
...
class S2Activation_SwiGLU_MemoryEfficient(S2Activation_SwiGLU):
    ...
    def forward(self, inputs):
        outputs = torch.utils.checkpoint.checkpoint(
            self.kernel, inputs, use_reentrant=False
        )
        return outputs
...
class SeparableS2Activation_SwiGLU_Merge_MemoryEfficient(
    S2Activation_SwiGLU_MemoryEfficient
):
    def __init__(
        self,
        lmax,
    ...
    def forward(self, inputs, scalars):
        outputs = torch.utils.checkpoint.checkpoint(
            self.kernel, inputs, scalars, use_reentrant=False
        )
        return outputs
...
class SeparableGateS2Activation_SwiGLU_Merge(GateActivation):
    ...
    def forward(self, inputs, scalars):
        ...
        gate_scalars = scalars.narrow(
            2, output_scalars.shape[2], (scalars.shape[2] - output_scalars.shape[2])
        )
...
class SeparableGateS2Activation_SwiGLU_Merge_MemoryEfficient(
