...
from .layer_norm import RMSNorm, get_normalization_layer
from .radial_function import RadialFunction
from .so2_ops import SO2Linear
from .so3 import SO3Grid, SO3Linear
...
class EquivariantGraphAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if not self.use_add_merge:
            if self.use_rad_l_parametrization:
                self.edge_channels_list.append(
                    (self.num_in_channels * (self.lmax + 1) * 2)
                )
            else:
        ...
        # Graph attention
        self.alpha_norm = (
            torch.nn.LayerNorm(self.attn_alpha_channels)
            if self.use_attn_renorm
            else torch.nn.Identity()
        )
        self.alpha_act = torch.nn.SiLU() if alpha_drop != 0.0 else SmoothLeakyReLU()
        self.alpha_dropout = (
            torch.nn.Dropout(alpha_drop) if alpha_drop != 0.0 else torch.nn.Identity()
        )
        ...
        self.attn_softmax = GraphSoftmax(
            eps=self.eps, exp_dropout=attn_mask_rate, softcap=self.softcap
        )
        self.attn_weights_dropout = (
            torch.nn.Dropout(attn_weights_drop)
            if attn_weights_drop != 0.0
            else torch.nn.Identity()
        )
        ...
        self.so2_linear_2 = SO2Linear(
            (
                ...
                else (self.num_hidden_channels // 2)
            ),
    ...
    def forward(
        ...
        if not self.use_add_merge:
            ...
            x_edge_weight_target = x_edge_weight.narrow(
                2, self.num_in_channels, self.num_in_channels
            )
        ...
        if has_scalars(self.activation):
            ...
            x_scalar = x_m0_extra.narrow(
                1, self.split_m0_channels_list[0], self.split_m0_channels_list[1]
            )
        else:
            x_alpha = x_m0_extra
            x_scalar = None
...
class FeedForwardNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        self.so3_linear_1 = SO3Linear(
            self.num_in_channels, self.num_hidden_channels, lmax=self.lmax
        )
        ...
        if self.use_grid_mlp:
            ...
            if "swiglu" in self.activation:
                if "gates2" not in self.activation:
                    ...
                    self.grid_mlp = GatedSwiGLUGridMLP(
                        self.num_in_channels, self.num_hidden_channels, dropout
                    )
            elif "square" in self.activation:
                self.grid_mlp = torch.nn.Sequential(
                    LinearSquare(
                        self.num_hidden_channels, self.num_hidden_channels, bias=False
                    ),
                    (
                        torch.nn.Dropout(dropout)
                        if dropout > 0.0
            ...
            if self.activation == "gate":
                self.gating_linear = torch.nn.Linear(
                    self.num_in_channels, (self.lmax * self.num_hidden_channels)
                )
            elif self.activation == "sep_s2":
                self.gating_linear = torch.nn.Linear(
                    self.num_in_channels, self.num_hidden_channels
                )
            elif self.activation == "sep-merge_gates2_swiglu":
                del self.so3_linear_1
                self.so3_linear_1 = SO3Linear(
                    self.num_in_channels, 2 * self.num_hidden_channels, lmax=self.lmax
                )
                self.gating_linear = torch.nn.Linear(
        ...
        self.so3_linear_2 = SO3Linear(
            self.num_hidden_channels, self.num_out_channels, lmax=self.lmax
        )
        if "-merge" in self.activation:
...
class GatedSwiGLUGridMLP(torch.nn.Module):
    def __init__(self, num_in_channels, num_hidden_channels, dropout):
        ...
        self.grid_linear_1 = torch.nn.Linear(
            self.num_hidden_channels, 2 * self.num_hidden_channels, bias=False
        )
        self.grid_drop = (
        ...
        self.grid_linear_2 = torch.nn.Linear(
            self.num_hidden_channels, self.num_hidden_channels, bias=False
        )
...
class TransBlockV3(torch.nn.Module):
    ...
    def __init__(
        ...
        self.proj_drop = (
            EquivariantDropout(lmax=lmax, mmax=lmax, drop_prob=proj_drop)
            if proj_drop > 0.0
            else None
        )
