...
class ScalarFeedForwardNetwork(torch.nn.Module):
    ...
    def __init__(
        self, num_in_channels, num_hidden_channels, num_out_channels, dropout=0.0
    ):
        super().__init__()
        ...
        self.linear_1 = torch.nn.Linear(
            self.num_in_channels, self.num_hidden_channels, bias=True
        )
        ...
        self.linear_2 = torch.nn.Linear(
            self.num_hidden_channels, self.num_out_channels, bias=True
        )
...
class EquivariantGraphAttentionStressHead(EquivariantGraphAttention):
    ...
    def forward(
        ...
        stress = torch.einsum(
            "ni, ij -> nj", stress, self.zero_padded_change_matrix
        )  # (batch_size, 9)
        return stress
