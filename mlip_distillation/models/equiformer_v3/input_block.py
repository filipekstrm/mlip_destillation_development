...
class EdgeDegreeEmbedding(torch.nn.Module):
    ...
    def forward(
        self, atomic_numbers, edge_distance, edge_index, edge_envelope_weight=None
    ):
        if self.use_atom_edge_embedding:
        ...
        x_edge_m0 = x_edge_m0.view(
            x_edge_m0.shape[0], (self.lmax + 1), self.num_channels
        )
        x_edge = torch.bmm(
            self.so3_rotation.wigner_inv.narrow(2, 0, (self.lmax + 1)), x_edge_m0
